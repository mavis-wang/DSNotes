...
raw_cloth_DIR = data_raw_DIR + 'cloth'

humanParsingDIR = '../lib/humanParsing/'
cpVtonPlusDIR = '../lib/cpVtonPlus/'

# tryon inputs
img_frames_DIR = os.path.join(data_processed_DIR, 'image/')
cloth_DIR = os.path.join(data_processed_DIR, 'cloth/')
parsed_img_DIR = os.path.join(data_processed_DIR, 'image-parse/')
image_mask_DIR = os.path.join(data_processed_DIR, 'image-mask/')
img_parse_new_DIR = os.path.join(data_processed_DIR, 'image-parse-new/')
cloth_mask_DIR = os.path.join(data_processed_DIR, 'cloth-mask/')
pose_DIR = os.path.join(data_processed_DIR, 'pose/')

parsed_img_lip_DIR = os.path.join(data_processed_DIR, 'image-parse-lip/')
input_cloth_DIR = os.path.join(input_DIR, 'cloth/')

# output shortcuts
out_tryOn_DIR = '../lib/cpVtonPlus/result/TOM/test/try-on'
out_warpCloth_DIR = '../lib/cpVtonPlus/result/GMM/test/warp-cloth'
out_tryOnimg_frames_DIR = os.path.join(results_DIR, 'tryOn-images/')
...
def toCocoJson(keypoints_list, pose_kp_DIR):    
    ...
    for keypoint in keypoints_list:
        ...
        keypoints_coco.append(COCOpose)
        fileName = str(count).zfill(6) + '_0_keypoints.json'
        fPath = pose_kp_DIR + fileName
        with open(fPath, 'w') as file:
            json.dump(COCOpose, file)
        count += 1
...
def getKP(videoPath):
    ...
    while hasFrame:  
        ...
        if hasFrame:
            ...
            net.setInput(
                cv.dnn.blobFromImage(frame, 1.0, 
            ...
            for i in range(len(BODY_PARTS)):
                ...
                xyv.append((int(x), int(y), conf > 0.3))
                
            xyv_flatten = (np.array(xyv)).flatten().tolist()
            keypoints.append(xyv_flatten)
            ...
            for pair in POSE_PAIRS:
                ...
                if points[idFrom] and points[idTo]:
                    cv.line(frame, points[idFrom], points[idTo], (223, 150, 21), 2)
                    cv.ellipse(frame, points[idFrom], (3, 3), 0, 0, 360, (77, 40, 250), cv.FILLED)
                    cv.ellipse(frame, points[idTo], (3, 3), 0, 0, 360, (77, 40, 250), cv.FILLED)
            
            # kp overlay
            output_kps_overlay.append(frame)
            savePath = os.path.join(results_DIR, 'kps_overlay/')
    ...
    toCocoJson(keypoints, pose_DIR)
    
    return keypoints, output_kps_overlay
...
""" CP-VTON-PLUS
"""
def tryOn(filePath = '../lib/cpVtonPlus/test.py',
         name = 'GMM', dataroot = '../lib/cpVtonPlus/data',
         datamode = 'test', stage = 'GMM',
         pairlist = '../lib/cpVtonPlus/data/testPairs.txt',
         checkpoint = '../lib/cpVtonPlus/models/gmm_final.pth', worker='1'):
